...
export function resolveAuthProfileOrder(params: {
  ...
  const orderedAvailable = available.toSorted((a, b) => {
    const aUsage = params.store.usage[a]?.lastUsed ?? 0;
    const bUsage = params.store.usage[b]?.lastUsed ?? 0;
    return aUsage - bUsage;
  });
  const orderedCooledDown = cooledDown.toSorted((a, b) => a.until - b.until);

  const ordered = [...orderedAvailable, ...orderedCooledDown.map((entry) => entry.id)];
  if (params.preferredProfile && ordered.includes(params.preferredProfile)) {
    return {
      ...
      cooledDownProfileIds: orderedCooledDown.map((entry) => entry.id),
    };
  }
